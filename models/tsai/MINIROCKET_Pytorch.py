...
def get_minirocket_features(o, model,device="cpu", chunksize=1024, use_cuda=None, to_np=True):
    ...
    for oi in torch.split(o, chunksize):
        ...
        # ===================noise===================== #
        gamma = np.random.uniform(low=0.0, high=0.8)
        beta = np.round( np.random.uniform(low=1,high=7.5) ).astype(int)
        corrupted_perc = 0.0
        while corrupted_perc < gamma:
            ch = np.random.randint(0,oi.shape[1])
            pos = np.random.randint(0, oi.shape[-1]-beta)
            oi[:, ch, pos : pos+beta] = torch.normal(mean=0.0, std=torch.tensor([1.0]*beta))
            corrupted_perc +=  beta / (oi.shape[1]*oi.shape[-1])
